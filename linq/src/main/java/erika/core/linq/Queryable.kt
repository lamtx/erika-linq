...
fun <T : Expressible> Queryable<T>.limit(limit: Int, offset: Int = 0) =
    LimitedQueryable(this, limit, offset)

fun <T : Expressible, Key : Any> Queryable<T>.groupBy(selector: (T) -> Key) =
    GroupQueryable(this, selector(source))

fun <T : Expressible> Queryable<T>.orderBy(block: ((T) -> Array<out Expressible>)) =
    OrderedQueryable(this, block(source))
